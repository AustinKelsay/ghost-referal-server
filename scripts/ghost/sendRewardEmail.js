...
const sendRewardEmail = async (email, referrerEmail) => {
    ...
    try {
      ...
      const refereeEmailResponse = await axios.put(`${GHOST_API}/posts/${postId}/?email_segment=email:"${email}"`, {
        posts: [
          {
            status: 'published',
      ...
      if (referrerEmail) {
        ...
        await axios.put(`${GHOST_API}/posts/${referrerPostId}/?email_segment=email:"${referrerEmail}"`, {
          posts: [
            {
              status: 'published',
